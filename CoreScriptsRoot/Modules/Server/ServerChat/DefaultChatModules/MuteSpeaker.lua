	...
	local function DoMuteCommand(speakerName, message, channel)
		...
		if speaker then
			...
			if muteSpeaker then
				...
				speaker:SendSystemMessage(string.format("Speaker '%s' does not exist.", muteSpeaker.Name), channel, errorExtraData)
			end
		end
	end
	...
	local function DoUnmuteCommand(speakerName, message, channel)
		...
		if speaker then
			...
			if unmuteSpeaker then
				...
				speaker:SendSystemMessage(string.format("Speaker '%s' does not exist.", unmuteSpeaker.Name), channel, errorExtraData)
			end
		end
	end
